...
echo -e " Name: \x1b[1m\x1b[33m$(echo $i | cut -d, -f1 2>/dev/null)\x1b[0m"
echo " Duration: $(echo $i | cut -d, -f2 2>/dev/null)"
echo " URL: $(echo $i | cut -d, -f3 2>/dev/null)"
echo "$track $(echo $i | cut -d, -f3 2>/dev/null)" 2>/dev/null >>/tmp/ytpp3_urls
done
done
...
while :; do
	...
	if [[ "$ntrack" =~ ^[1-9]$ ]] || [[ "$ntrack" == "10" ]]; then
		link_to_play=$(cat /tmp/ytpp3_urls 2>/dev/null | grep "^$ntrack " 2>/dev/null| cut -d " " -f2 2>/dev/null)
		echo "Playing Track $ntrack..."
		vlc -I "dummy" "$link_to_play" --no-video --no-loop --no-repeat --play-and-exit 2>/dev/null 3>/dev/null >/dev/null
	elif [[ "$ntrack" == "0" ]]; then
